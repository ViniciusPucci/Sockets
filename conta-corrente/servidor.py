...
while 1:
	...
	operacao_str = conn.recv(BUFFER_SIZE).decode("utf-8")
	if not operacao_str: break
	print ("[SERVIDOR] Dados recebidos do cliente com sucesso: \"" + operacao_str + "\"")
	
	operacao = operacao_str.strip().split(' ')
	if operacao[0] == "SALDO":
		resposta = str(saldo)
